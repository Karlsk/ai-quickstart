...
if response.choices[0].message.tool_calls:
    ...
    print(f"模型请求调用函数 {func_name}")
    print(f"函数参数: {function_call_arguments}, type: {type(function_call_arguments)}")
    function_to_call = available_functions[func_name]
    result = {"horoscope": function_to_call(**function_call_arguments)}
